...
Только stdlib (без sklearn/numpy): канонизация синонимов, стоп-слова,
униграммы + биграммы слов + символьные триграммы (ловят словоформы и
перефразировки без стеммера), сглаженный IDF, косинусная близость.
Ничего не размечает автоматически — только показывает
...
# Вежливость и служебные слова: шум для схожести (отрицания не трогаем!).
STOP_RU = frozenset("""
здравствуйте здравствуй добрый день вечер утро привет пожалуйста спасибо
благодарю извините подскажите подсказать скажите сказать меня тебя себя
мой моя моё мои ваш ваша ваше ваши это эта эти как что чтобы или если
уже очень просто такой такая там тут здесь вот все меня нам вам есть
будет было будут
""".split())

# Канонические группы: одно значение разными словами (только безопасные).
_SYNONYMS = (
    ("обмен", ("обмен", "обменять", "перенести", "перенос", "поменять",
               "переоформить", "перебронировать")),
    ("возврат", ("возврат", "вернуть", "возвратить", "сдать", "рефанд")),
    ("рейс", ("рейс", "перелет", "перелёт", "вылет")),
    ("багаж", ("багаж", "чемодан")),
    ("отель", ("отель", "гостиница")),
    ("деньги", ("деньги", "средства")),
    ("срок", ("срок", "сроки")),
    ("отмена", ("отмена", "отменить", "аннулировать")),
    ("болезнь", ("болезнь", "болеть", "заболеть")),
    ("нельзя", ("нельзя", "невозможно")),
    ("можно", ("можно", "возможно", "разрешено")),
    ("поддержка", ("поддержка", "оператор")),
    ("цена", ("цена", "стоимость")),
    ("паспорт", ("паспорт", "загранпаспорт")),
    ("хотеть", ("хочу", "хотите", "желаю", "желаете")),
)
_SYN_MAP = {w: canon for canon, words in _SYNONYMS for w in words}
...
def _terms(text: str) -> list:
    """Униграммы + биграммы слов (канонизированные, без стоп-слов)
    + символьные триграммы (словоформы и опечатки без стеммера)."""
    toks = [_SYN_MAP.get(t, t) for t in _tokens(text)]
    toks = [t for t in toks if t not in STOP_RU]
    terms = list(toks)
    terms.extend(f"{a} {b}" for a, b in zip(toks, toks[1:], strict=False))
    for t in toks:
        for i in range(len(t) - 2):
            terms.append("~" + t[i:i + 3])
    return terms
