...
def bulk_set_status(project_path: str, case_ids: list, status: str) -> int:
    ...
    with db(project_path) as conn:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO bulk_operations (op_type, params_json, case_count, created_at) VALUES (?, ?, ?, ?)",
            ("set_status", json.dumps({"status": status}, ensure_ascii=False), len(case_ids), now),
        )
        op_id = cur.lastrowid
        done = 0
        for chunk in _chunked(case_ids):
            ...
            for cid in chunk:
                ...
                cur.execute("""
                    INSERT INTO history (case_id, event_type, field_name, old_value, new_value, created_at, operation_id)
                    VALUES (?, 'status_changed', 'status', ?, ?, ?, ?)
                """, (cid, old_status, status, now, op_id))
                cur.execute("""
                    INSERT INTO bulk_operation_items (operation_id, case_id, field_name, old_value, new_value)
                    VALUES (?, ?, 'status', ?, ?)
                """, (op_id, cid, old_status, status))
                done += 1
        cur.execute("UPDATE bulk_operations SET case_count=? WHERE operation_id=?", (done, op_id))
    logger.info("bulk set_status=%s done=%s/%s op=%s", status, done, len(case_ids), op_id)
    return done
        ...
        for chunk in _chunked(case_ids):
            for cid in chunk:
                ...
                if cur.rowcount > 0:
                    ...
                    """, (cid, str(tag_id), now, op_id))
                    cur.execute("""
                        INSERT INTO bulk_operation_items (operation_id, case_id, field_name, old_value, new_value)
                        VALUES (?, ?, 'tag_added', NULL, ?)
                    """, (op_id, cid, str(tag_id)))
                    done += 1
        cur.execute("UPDATE bulk_operations SET case_count=? WHERE operation_id=?", (done, op_id))
    return done
...
def undo_bulk_operation(project_path: str, operation_id: int) -> int:
    ...
    with db(project_path) as conn:
        ...
        if row["undone"]:
            return 0
        items = cur.execute(
            "SELECT case_id, field_name, old_value, new_value FROM bulk_operation_items WHERE operation_id=?",
            (operation_id,),
        ).fetchall()
        done = 0
        for it in items:
            cid, field, old, _new = it["case_id"], it["field_name"], it["old_value"], it["new_value"]
            if field == "status":
                cur.execute("""
                    INSERT INTO annotations (case_id, status, updated_at)
                    VALUES (?, ?, ?)
                ...
                cur.execute("DELETE FROM case_tags WHERE case_id=? AND tag_id=?", (cid, int(_new)))
            else:
                continue
            cur.execute("""
                INSERT INTO history (case_id, event_type, field_name, old_value, new_value, created_at, operation_id)
                VALUES (?, 'bulk_undone', ?, ?, ?, ?, ?)
            """, (cid, field, _new, old, now, operation_id))
            done += 1
        cur.execute("UPDATE bulk_operations SET undone=1 WHERE operation_id=?", (operation_id,))
